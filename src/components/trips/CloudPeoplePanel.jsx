...
export function CloudPeoplePanel({ trip, currentUserId }) {
  const people = useCloudTripPeople(trip)
  const [inviteOpen, setInviteOpen] = useState(false)
  const canInvite = isCloudTripOwner(trip, currentUserId)

  if (!trip?.id) {
    return <p className="text-sm text-ink-muted">People on this cloud trip could not be loaded just now.</p>
  }
...
function PendingCloudInvite({ invitation, canManage, onRevoke }) {
  ...
  return (
    <li className="flex flex-wrap items-start justify-between gap-3 py-4 first:pt-0 last:pb-0">
      ...
      {canManage ? (
        confirm ? (
          <button type="button" className="text-sm text-accent" onClick={withdraw}>
            Withdraw invite?
          </button>
        ) : (
