...
function initialTripId(expense, defaultTripId, trips) {
  return expense?.tripId ?? defaultTripId ?? trips[0]?.id ?? ''
}

function initialParticipantIds(expense, trip) {
  if (expense) return expense.shares.map((share) => share.userId)
  return trip?.members.map((member) => member.userId) ?? []
}
...
export function ExpenseForm({
  ...
  const converted = convert(total, currency, trip?.currency ?? HOME_CURRENCY)
  ...
  function applyTrip(nextTripId) {
    setTripId(nextTripId)
    if (expense) return
    const nextTrip = trips.find((item) => item.id === nextTripId)
    const ids = nextTrip?.members.map((member) => member.userId) ?? []
    setParticipantIds(ids)
    setPayerId(ids.includes(currentUser.id) ? currentUser.id : ids[0] ?? currentUser.id)
    setCurrency(nextTrip?.currency ?? HOME_CURRENCY)
    setShareDrafts(ids.length === 1 ? { [ids[0]]: amount } : {})
  }
    ...
    <form id="expense-form" onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {members.map((member) => {
            const selected = member.userId === payerId
            return (
              <button
                key={member.userId}
                type="button"
              ...
              >
                <Avatar initials={member.user?.initials ?? '?'} size="sm" />
                <span className="text-[13px]">{displayName(member.user, currentUser.id)}</span>
              </button>
            )
          })}
        </div>
      ...
      <div>
        ...
        <ul className="divide-y divide-line border-y border-line">
          {members.map((member) => {
            const included = participantIds.includes(member.userId)
            return (
              <li key={member.userId} className="flex items-center gap-3 py-3">
                <button
                  ...
                  aria-label={`${included ? 'Remove' : 'Include'} ${displayName(member.user, currentUser.id)}`}
                >
                  {included ? <IconCheck className="h-3.5 w-3.5" /> : null}
                </button>
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-ink">{displayName(member.user, currentUser.id)}</p>
                  {member.former ? (
                    <p className="text-[12px] text-ink-subtle">No longer on the trip</p>
                  ) : null}
                </div>
                {included ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      inputMode="decimal"
                      value={shareDrafts[member.userId] ?? ''}
                      onChange={(event) =>
                        setShareDrafts((current) => ({ ...current, [member.userId]: event.target.value }))
                      }
                      placeholder="0"
                    />
                    <button
                      ...
                      onClick={() => assignRemainder(member.userId)}
                    >
                      Rest
                    </button>
                  </div>
